    ...
    def parse_items():
        ...
        for i in xml_items:
            ...
            export_item = {
                ...
                'all_day': gi('wp:postmeta-_event_all_day'),
...
def write_jekyll(data, target_format):
    ...
    for i in data['items']:
        ...
        with open('authors.txt', 'r') as fin:
            authors_list = eval(fin.read())
        for wp_id_num,wp_login,wp_slug in authors_list:
            if i['author'] == wp_login:
                author = wp_slug
        sys.stdout.write('.')
        sys.stdout.flush()
        out = None
        yaml_header = {
            'title': i['title'],
            'author': author,
            'published-date': datetime.strptime(
